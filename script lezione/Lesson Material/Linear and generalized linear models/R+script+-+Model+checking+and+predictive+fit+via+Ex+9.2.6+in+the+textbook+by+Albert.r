...
X11()
par(mfrow=c(1,2))
#La distr. FINALE di tau=1/sigma^2 è gamma((n-p)/2, ((n-p)/2)*S^2)
#Ecco l'istogramma:
...
#Le medie e le mediane a posteriori dei beta (che coincidono perchè
#le distrib finali sono simmetriche) sono uguali alle stime MLE - a meno
#di errori di simulazione - perchè abbiamo usato la prior di Jeffreys
...
# Vogliamo stimare E(y_new|beta, x1)=x_1*beta, dove x_1 è una "nuova" matrice di covariate;
# si noti che questa è semplicemente  una combinazione lineare di beta.
cov1=c(1,4,0,0) #A
cov2=c(1,4,1,0) #B
cov3=c(1,4,0,1) #C
cov4=c(1,4,1,1) #D
...
# Prima abbiamo simulato la distribuzione finale di E(y_new|X1) - che è una funzione dei 4 prametri di regressione.
# Ora invece vogliamo la distribuzione predittiva di y_new, cioè
...
#in ognuno dei valori di theta generati dalla posterior, cioè generata da una 
...
# più in generale, ogni osservazione che NON è stata prodotta dal meccanismo che ha 
# generato la maggior parte dei dati"
# A priori la prob. che una osservazione sia un outlier è 
# P(|eps_i|/sigma>k)=2Phi(-k); bisognerà calcolare A POSTERIORI la prob di questo 
# evento, e confrontarla con quella a priori
# NON è un metodo molto CONVINCENTE...
...
#help(bayesresiduals)
##Calcoliamo la probabilità predittiva che una osservazione sia classificata
##come OUTLIER con k=2 - formula di Chaloner & Brant (1988)
#
#prob.out=bayesresiduals(fit,theta.sample,2)
#par(mfrow=c(1,1))
#plot(nesting,prob.out,cex=1)
##Ritengo OUTLIER quelle specie per cui queste probabilità sono maggiori di 
##0.35, per esempio
##out = (prob.out > 0.35)
#
...
# è una caso particolare della coniugata, e quindi riesco a simulare un campione iid dalla posterior,
